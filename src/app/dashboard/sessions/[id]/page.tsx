...
export default async function SessionDetailPage({ params }: { params: { id: string } }) {
  ...
  const rawSession = await Session.findById(params.id).lean().exec();
  if (!rawSession) {
    return (
      <main className="min-h-screen p-8">
        <div className="max-w-3xl mx-auto">
          <p className="text-red-600">Session not found.</p>
          <Link href="/dashboard/sessions" className="text-sm text-blue-600 underline">← Back to sessions</Link>
        </div>
      </main>
    );
  }
  ...
  const players: PlayerRow[] = rawPlayers.map((p) => ({ _id: String(p._id), name: String(p.name), jersey: p.jersey ?? undefined }));
    ...
    <main className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white shadow rounded p-4">
          <EditSessionAttendees sessionId={sessionData._id} players={players} initialAttendees={sessionData.attendees} />
        </div>
      </div>
    </main>
  );
